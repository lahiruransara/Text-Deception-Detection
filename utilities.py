...
def readFilesFromSources(text, sources):
    ...
    for source in sources:
        with open(source, encoding='utf8', errors='ignore') as f_input:
            text.append(f_input.read())


def ngram_transform(train_txt, valid_txt, n, stopwords=None, max_Features=None):
    tfidf_vect_ngram = TfidfVectorizer(analyzer='word', token_pattern=r'\w{1,}', ngram_range=(1, n), lowercase=True,
